...
const safeIcon = (
  name: string,
  fallback: IoniconsName = "ellipse",
): IoniconsName => ICON_MAP[name] ?? fallback;
...
const TipCard: React.FC<{ tip: Tip }> = ({ tip }) => (
  <View style={styles.tipCard}>
    <View
      style={[styles.tipIconBox, { backgroundColor: tip.iconColor + "20" }]}
    >
      <Ionicons
        name={safeIcon(tip.icon, "flash")}
        size={21}
        color={tip.iconColor}
      />
    </View>
...
const ElectricityTheftSection: React.FC<{ data: ElectricityTheftData }> = ({
  data,
}) => (
  <View style={styles.card}>
    ...
    >
      <Text style={styles.primaryBtnText}>
        {data.learnMoreLabel ?? "Learn More"}
      </Text>
    </TouchableOpacity>
  </View>
);

const EnergySavingSection: React.FC<{ data: EnergySavingData }> = ({
  data,
}) => (
  <View style={styles.card}>
    {/* Header */}
...
const SafetyGuidelinesSection: React.FC<{ data: SafetyGuidelinesData }> = ({
  data,
}) => (
  <View style={styles.card}>
    {/* Header */}
...
export default function LearnScreen(): React.ReactElement {
  ...
  const fetchSections = useCallback(async (): Promise<void> => {
    try {
      ...
      if (snapshot.size < 3) {
        console.log(
          `Only ${snapshot.size} docs found — seeding missing data...`,
        );
        ...
        if (seeded) {
          ...
          setSections(
            snapshot2.docs.map(
              (d) => ({ id: d.id, ...d.data() }) as SectionData,
            ),
          );
        }
      } else {
        setSections(
          snapshot.docs.map((d) => ({ id: d.id, ...d.data() }) as SectionData),
        );
      }
    } catch (err) {
