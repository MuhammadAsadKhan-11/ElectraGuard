...
// ─────────────────────────────────────────────────────────────
// SYSTEM SETTINGS MODAL  (Language + Theme)
// ─────────────────────────────────────────────────────────────
const SystemSettingsModal: React.FC<{
  visible: boolean;
  onClose: () => void;
}> = ({ visible, onClose }) => {
  const { t, language, theme, setLanguage, setTheme } = useAppSettings();
...
const RolesModal: React.FC<{
  ...
  const allPermissions = [
    ...
    },
    {
      ...
      icon: "cloud-upload-outline" as IoniconsName,
    },
    {
      ...
      <View style={styles.modalOverlay}>
        <View style={[styles.modalBox, { paddingBottom: 28 }]}>
          ...
          {allPermissions.map(({ key, label, icon }) => {
            const hasPermission =
              granted.includes(key) || profile?.role === "super_admin";
            return (
              <View key={key} style={styles.permRow}>
                <View style={styles.menuIconBox}>
                  <Ionicons name={icon} size={16} color="#718096" />
                </View>
                <Text style={[styles.menuLabel, { flex: 1, fontSize: 13 }]}>
                  {label}
                </Text>
                <View
                  style={[
                    styles.permBadge,
                    { backgroundColor: hasPermission ? "#E6F4EA" : "#FFF0F0" },
                  ]}
                >
                  <Ionicons
          ...
          {[
            {
              ...
              body: "ElectraGuard collects admin credentials, case management data, and consumer records to provide electricity monitoring services.",
            },
            {
              heading: "2. Data Usage",
              body: "Admin data is used solely for system management, case resolution, and theft detection. We do not sell or share data with third parties.",
            },
            {
            ...
            },
            {
              ...
              body: "Firebase Authentication is used for secure admin login. Role-based access controls data visibility.",
            },
            {
              heading: "5. Your Rights",
              body: "Admins may request account deletion by contacting the system owner. All associated data will be permanently removed.",
            },
            {
              heading: "6. Security",
              body: "We implement industry-standard security including encrypted transmission, hashed passwords, and regular audits.",
            },
            {
              heading: "7. Contact",
              body: "For queries, contact support@electraguard.pk or through the Support section.",
            },
          ].map(({ heading, body }) => (
            <View key={heading} style={{ marginBottom: 14 }}>
              ...
              <Text style={{ fontSize: 12, color: "#4A5568", lineHeight: 18 }}>
                {body}
              </Text>
            </View>
          ))}
        ...
        <View style={styles.card}>
          <MenuRow
            ...
            onPress={() =>
              Alert.alert(
                "Notifications",
                "You will receive push notifications for new cases, alerts, and consumer risk updates.",
                [{ text: "OK" }],
              )
            }
          />
