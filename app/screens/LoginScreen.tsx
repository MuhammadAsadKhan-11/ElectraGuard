...
const sendEmail = async (
  ...
  if (!response.ok) throw new Error(`EmailJS ${response.status}: ${responseText}`);
};
    ...
    try {
      ...
      if (activeTab === 'Admin') {
        ...
        const adminSnap = await getDocs(
          query(collection(db, 'admins'), where('uid', '==', userCredential.user.uid))
        );
        ...
        try {
          ...
          Alert.alert('Email Error', emailError.message || 'Failed to send OTP email.');
          setLoading(false);
          return;
        }
...
const styles = StyleSheet.create({
  ...
  otpNoticeText:   { fontFamily: 'Inter_400Regular', fontSize: 12, color: '#0B3C5D', flex: 1 },
