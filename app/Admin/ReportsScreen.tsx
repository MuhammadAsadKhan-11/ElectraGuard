...
const DonutChart: React.FC<{ slices: PieSlice[]; size: number }> = ({ slices, size }) => {
  const radius = size / 2;
...
const PieSegmentsView: React.FC<{ slices: PieSlice[]; size: number }> = ({ slices, size }) => {
  const half = size / 2;

  let rotationDeg = 0;
  return (
    <View style={{ width: size, height: size, borderRadius: half, overflow: "hidden", backgroundColor: "#E5E7EB" }}>
      {slices.map((slice, i) => {
...
const pieStyles = StyleSheet.create({
  ...
  segment: { height: "100%" },
  legend: { flexDirection: "row", gap: 14, marginTop: 14, flexWrap: "wrap", justifyContent: "center" },
...
// BAR CHART COMPONENT
// ─────────────────────────────────────────────────────────────
const BarChart: React.FC<{ data: BarData[]; color: string; yLabel?: string }> = ({
  data,
  color,
  yLabel,
}) => {
...
export default function ReportsScreen(): React.ReactElement {
  ...
  const PERIODS: FilterPeriod[] = ["Last 7 days", "Last 30 days", "Last 3 months"];
        ...
        <View style={styles.headerCard}>
          <View style={styles.headerTop}>
            ...
            <TouchableOpacity style={styles.bellBtn} activeOpacity={0.7}>
              <Ionicons name="notifications-outline" size={20} color="#0B3C5D" />
            </TouchableOpacity>
          </View>
        </View>
...
function renderPieSegments(slices: PieSlice[], size: number) {
  ...
  slices.forEach((slice, i) => {
    ...
    if (slice.percent > 50) {
      // First half (0-180)
      elements.push(
        <View key={`${i}a`} pointerEvents="none" style={{ position: "absolute", width: size, height: size, transform: [{ rotate: `${startDeg}deg` }] }}>
          <View style={{ position: "absolute", width: half, height: size, left: half, overflow: "hidden" }}>
            <View style={{ position: "absolute", width: size, height: size, borderRadius: half, backgroundColor: slice.color, right: 0, transform: [{ rotate: "180deg" }] }} />
          </View>
        </View>
      );
      // Second half
      elements.push(
        <View key={`${i}b`} pointerEvents="none" style={{ position: "absolute", width: size, height: size, transform: [{ rotate: `${startDeg + 180}deg` }] }}>
          <View style={{ position: "absolute", width: half, height: size, left: half, overflow: "hidden" }}>
            <View style={{ position: "absolute", width: size, height: size, borderRadius: half, backgroundColor: slice.color, right: 0, transform: [{ rotate: `${endDeg - startDeg - 180}deg` }] }} />
          </View>
        </View>
      );
    } else {
      elements.push(
        <View key={i} pointerEvents="none" style={{ position: "absolute", width: size, height: size, transform: [{ rotate: `${startDeg}deg` }] }}>
          <View style={{ position: "absolute", width: half, height: size, left: half, overflow: "hidden" }}>
            <View style={{ position: "absolute", width: size, height: size, borderRadius: half, backgroundColor: slice.color, right: 0, transform: [{ rotate: `${endDeg - startDeg}deg` }] }} />
          </View>
        </View>
      );
    }
...
const styles = StyleSheet.create({
  ...
  statValue: { fontSize: 22, fontWeight: "900", color: "#1A202C", marginBottom: 2 },
