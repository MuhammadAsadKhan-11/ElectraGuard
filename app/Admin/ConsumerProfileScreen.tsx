...
const MiniLineChart = ({ data, color }: { data: { date: string; value: number }[]; color: string }) => {
  const maxVal = Math.max(...data.map(d => d.value));
  const minVal = Math.min(...data.map(d => d.value));
  const range = maxVal - minVal || 1;
  const getY = (v: number) => CHART_HEIGHT - ((v - minVal) / range) * (CHART_HEIGHT - 20) - 10;
  ...
  return (
    <View style={{ height: CHART_HEIGHT }}>
      <View style={StyleSheet.absoluteFill}>
        {data.map((d, i) => {
          if (i === 0) return null;
          const x1 = getX(i - 1), y1 = getY(data[i - 1].value);
        ...
        {data.map((d, i) => (
          <View key={`dot-${i}`} style={[styles.chartDot, {
            left: getX(i) - 4, top: getY(d.value) - 4,
            backgroundColor: color,
          }]} />
        ))}
      </View>
      <View style={[styles.xLabels, { top: CHART_HEIGHT - 2 }]}>
        {data.map((d, i) => (
          <Text key={i} style={styles.xLabel}>{d.date}</Text>
        ))}
      </View>
    </View>
  );
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          ...
          <View style={[styles.riskBadge, { backgroundColor: riskColor + '15', borderColor: riskColor + '40', borderWidth: 1 }]}>
          ...
          <View style={styles.profileMeta}>
            ...
            <View>
              ...
              <Text style={[styles.profileValue, {
                color: consumer.status === 'Active' ? Colors.success : Colors.danger
              }]}>{consumer.status}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.card}>
          <View style={styles.riskAnalysisHeader}>
            ...
            <Text style={[styles.riskPct, { color: riskColor }]}>{consumer.riskScore}%</Text>
          </View>
        ...
        <View style={styles.card}>
          ...
          <MiniLineChart data={consumer.consumptionHistory} color={Colors.primary} />
        </View>
            ...
            {consumer.theftFlags.map((flag, i) => (
              <View key={i} style={styles.flagRow}>
                ...
                <View>
                  ...
                  <Text style={styles.flagDate}>{flag.date} • <Text style={{ color: severityColor(flag.severity) }}>{flag.severity} Severity</Text></Text>
                </View>
              </View>
            ))}
          </View>
            ...
            consumerCases.map(c => (
              ...
              >
                <View>
                  ...
                  <Text style={styles.caseDesc} numberOfLines={1}>{c.description}</Text>
                  <Text style={styles.caseDate}>{c.createdAt}</Text>
                </View>
                <View style={[styles.caseBadge, { backgroundColor: getCaseStatusColor(c.status) + '20' }]}>
                  <Text style={[styles.caseBadgeText, { color: getCaseStatusColor(c.status) }]}>{c.status}</Text>
                </View>
              </TouchableOpacity>
            ))
          )}
        ...
        <View style={styles.actionRow}>
          ...
          <TouchableOpacity style={[styles.actionBtn, { backgroundColor: Colors.teal }]}>
...
const styles = StyleSheet.create({
  ...
  profileRow: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: 12, gap: 10 },
  ...
  profileMeta: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 },
  riskAnalysisHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  ...
  xLabels: { position: 'absolute', left: 0, right: 0, flexDirection: 'row', justifyContent: 'space-between' },
  xLabel: { fontSize: 8, color: Colors.textSecondary },
  flagRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 10, marginBottom: 12 },
  ...
  noCases: { fontSize: 14, color: Colors.textSecondary, textAlign: 'center', padding: 16 },
  caseRow: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: Colors.border,
  },
  caseNumber: { fontSize: 14, fontWeight: '700', color: Colors.text },
  caseDesc: { fontSize: 12, color: Colors.textSecondary, marginTop: 2, maxWidth: width - 140 },
